...
# Find and return the path to a token in the tree and return if token
# isn't found    
...
# Get value in tree at the end of the given path and return if path
# doesn't exist
...
# Get time value in tree and return if time isn't found 
def getTime(tree):
...
# Get playmode value in tree and return if playmode isn't found 
...
# Get left score in tree and return if playmode isn't found 
...
# Get right score in tree and return if playmode isn't found 
...
def getInfo(fn, info='playmode'):
    ...
    for t in reversed(RDSTrees):
        ...
        # only do once for the last line,
        # here we do not need to recover match state,
        # just block following sentence
        return info_dict
